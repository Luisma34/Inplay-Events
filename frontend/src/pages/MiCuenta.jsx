...
export default function MiCuenta() {
  ...
  const [actionType, setActionType] = useState(null); // "cancelReserva" o "cancelLiga"
  ...
  const handleSaveProfile = () => {
    alert("Guardado. En V2 se conecta al backend.");
  };

  // función para confirmar acción (cancelar reserva o salir de liga)
  const confirmarAccion = () => {
    if (actionType === "reserva") {
      fetch(`http://localhost:8080/api/reservas/${selectedId}`, {
        method: "DELETE",
        credentials: "include",
      })
        .then((res) => {
          if (!res.ok) throw new Error();
          setMyReservas((prev) => prev.filter((r) => r.id !== selectedId));
        })
        .catch(() => alert("Error al cancelar la reserva"));
    }

    if (actionType === "liga") {
      fetch(`http://localhost:8080/api/ligas/${selectedId}/salir`, {
        method: "DELETE",
        credentials: "include",
      })
        .then((res) => {
          if (!res.ok) throw new Error();
          setMisLigas((prev) => prev.filter((l) => l.id !== selectedId));
        })
  ...
  const handleCancelLiga = (ligaId) => {
    setActionType("liga");
    setSelectedId(ligaId);
    setShowConfirm(true);
  };

  // función para cancelar una reserva (solo si no está ya cancelada)
  const handleCancelReserva = (id) => {
    setActionType("reserva");
    setSelectedId(id);
    setShowConfirm(true);
  };
      ...
      <Modal show={showConfirm} onHide={() => setShowConfirm(false)} centered>
        ...
        <Modal.Body>
          {actionType === "reserva"
            ...
            : "¿Seguro que quieres salir de esta liga?"}
        </Modal.Body>
        <Modal.Footer>
